...
async def main(input_file, output_file, chunk_size=10000):
    ...
    with open(input_file, 'r') as f:
        ...
        chunk = []
        for row in reader:
            chunk.append(row[0])  # Assuming domain names are in the first column
            if len(chunk) == chunk_size:
                results = await process_chunk(chunk)
                write_results(output_file, results)
                chunk = []
        if chunk:  # Process the remaining domains
            results = await process_chunk(chunk)
            write_results(output_file, results)
